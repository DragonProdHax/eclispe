...
function createHomeGameCard(game, index) {
    ...
    card.innerHTML = `
        <div class="game-card-inner">
            <img src="${game.icon}" alt="${game.name}" class="game-card-image" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'">
...
function playGame(game) {
    ...
    if (gameTags) {
        gameTags.innerHTML = game.tags.map(tag => `<span class="game-tag">${tag}</span>`).join('');
    }
